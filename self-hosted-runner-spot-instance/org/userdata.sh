...
github_org="000000000000000000000000000000000000000000"
...
echo -e "GH_HOSTNAME=$HOSTNAME\nGH_ORGANIZATION=$github_org\nGH_RUNNER_TOKEN=$token\nGH_IDLE_TIMEOUT=$runner_timeout" > /actions-runner/.creds

# Create the runner and start the configuration experience
sudo -u ec2-user ./config.sh --url https://github.com/$github_org --token $token --name "spot-runner-$(hostname)" --unattended
